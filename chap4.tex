...
%FIXME: What do you mean by "exact direction"?  Let's discuss.
%FIXME: Tables are an important part of the semantics.  Mention those as well.
users have to exactly set or get a variable in FWLua by the exact direction, and functions in FWLua are lambdas.
%It is because we want to use FWLua to represent some basic mechanisms in Lua. This is good for analyzing Lua

%We will further discuss every syntax and semantic of FWLua in this chapter.
...
%Basically in Lua, expressions return values and most statements change the store.
%Here we just pretend that all new expressions, including expressions and statements in Lua, will return a value and change the store. Next, we will discuss every rule in detail.
...
%FIXME: The discussion of this rule needs revision -- let's discuss
