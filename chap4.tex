...
% FIXME: We need to merge this with the previous chapters, so this intro may need to be deleted
In the previous chapters, we have introduced several primary parts in Lua. Expressions and statements are simple to understand because they can be analyzed in a straightforward way.
%, directly related to their syntax.  However, there is still a big part in the project, which would be also very important and need to be further analyzed in the certain extension. In other words, because simply reducing syntax about statements and expression would not be so hard, the core work we mainly focus in the project is about some more essential stuff.
...
%Basically, the definition of function is pretty straightforward: a function will take some arguments users already defined into its body thus form the closure body of the function, and then this defined function could take some values as perimeters when it is invoked for computation.
%
...
%
%There are some instances showing the code in the full version of Lua about functions.
...
\end{verbatim}
%Basically, either of the codes above delivers the same task. Also,
...
However,
%concerning about FWLua, the fact of multiple expressions might be unnecessary. It is because that
we want to keep the syntax for FWLua as simple we can. Therefore, we represent multiple expressions through desugaring, which we discuss later in this chapter.
