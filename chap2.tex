...
There are only a few default functions in the metatable. They include hooks for behavior controlling(also can be called as {\tt Fallback} in the previous version of Lua), and functions for binary operations. In addition, these metamethods can be redefined by the programmer. As a set of automatically executing functions, metatables can be very powerful and a nice convenience for Lua programmers.
...
%-------MODIFIED
There is also one thing worth mention: although metatables can control basic behaviors of a table, such as setting a fallback table, it is different with scopes and closures in functions. Metatables serve for tables, and tend to focus on dealing values and variables in  tables. The scoping and closures in functions, which are also handling variables and values, tend to focus on building scopes an hirearchies.
%-------END
