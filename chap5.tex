...
{\bf Translation:~~~ \fbox{$\llbracket e \rrbracket~:~{\tt Lua}~\xlongequal[]{}~{\tt FWLua}   $}   } 
...
\translateline{
    {\tt rawset}({\tt rawget}(a, ``t"), ``\_metatable", default);\\
    {\tt rawset}({\tt rawget}(a, ``t"), ``\_index", 0);
}
...
\translate{e_1[e_2]}{{\lambda}e_1.{\lambda}e_2.}
\translateline{ 
...
\desugar{{\tt local}~x=e}{a.\_localscope.x = e;}
...
\desugar{t = \{e_1=e_2\}}{t = \{\}; t[e_1] = e_2}
\desugar{t = \{\overline{e_i = e_j}\}}{t = \{\}; t[e_i]=e_j; t }
\desugar{t = \{x\}}{t = \{\};}
\desugarline{ {\tt while}~t[\_index] == {\tt nil~do}}
\desugarline{ ~~\_index = \_index + 1~ }
\desugarline{{\tt end};}
\desugarline{ t[\_index] = x}
\desugar{{\tt while}~e_1~{\tt do}~e_2~{\tt end}}{
    f =  {\tt if}~e_1~{\tt then}~e_2~{\tt else}~{\tt False}~{\tt end};
}
\desugarline{ {\tt if}~e_1~{\tt then}~{\tt f}~{\tt else}~{\tt False}~{\tt end}}
\desugar{
    e_1~ op ~e_2
}{
